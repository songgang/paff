...
function v = v_paff_ex_pqvec_multibeam(g, t, y, cps)
...
for ii = 1:g.nb_cps
    logwlist(ii,:) = loggpaff(y - squeeze(cps(:, :, ii)), g.aff{g.ind(ii)}.s);
end;
...
if 1
    ...
    for ii = 1:g.nb_cps
        ...
        wii = 1 ./ sw;
        affL = g.aff{g.ind(ii)}.L;
        affv = g.aff{g.ind(ii)}.v;
    ...
    for ii = 1:g.nb_cps
        ...
        viiy = g.aff{g.ind(ii)}.L * y + g.aff{g.ind(ii)}.v*ones(1, nb_y);
        v = v + (ones(g.dim, 1)*wii).*viiy;
    end;
